pub mod tests {
    ...
    use crate::rwtransaction_wrapper::{ReplicatedTxn, ValueWithMVCC, TypedValue};
        ...
        let clos = |ctx: Arc<DbContext>| {
            ...
            while i < 500 {
                ...
                if let Ok(range) = range {
                    let max = range.into_iter().map(|(_, v)| v.get_val().as_str().parse::<u64>().unwrap()).max().unwrap() + 1;
    ...
    pub fn monotonic() {
        ...
        fn add(ctx: &DbContext) {
            let retry = || {
                ...
                let len = range.len();
                let max = range.into_iter().map(|(_, v)| v.get_val().as_str().parse::<u64>().unwrap()).max().unwrap_or(0) + 1;
                if (max - 1) != len as u64 {
                    panic!("bad");
                }
        ...
        fn check(mut a: Vec<(ObjectPath, ValueWithMVCC)>) {
            ...
            for mut elem in a {
                ...
                if val.1.as_str().parse::<i64>().unwrap() <= prevvalue.as_str().parse::<i64>().unwrap() {
                    println!("{:?}", a_);
                    panic!()
                }
        ...
        let ctx = db!();
        scope(|s| {
            s.spawn(|_| for _ in 0..10 {
                std::thread::sleep(Duration::from_millis(250));
                read(&ctx)
            });
            s.spawn(|_| for _ in 0..200 {
...
pub mod tests_walwatcher {
    ...
    pub fn test2() {
        ...
        assert_eq!(auto_commit::read(&db, &"k".into()).unwrap().into_inner().1.as_str(), "2500");
    }
    ...
    pub fn test1() {
                ...
                let res = match res {
                    Err(err) => {
                        if err == *"Other(\"Read value doesn't exist\")" {
                            txn.write(&key.as_str().into(), TypedValue::from("1")).map(|_| ())
                        } else {
        ...
        scope(|s| {
            ...
            let checker = s.spawn(|_| while !state.load(Ordering::SeqCst) {
                ...
                check_func(&db).map_err(|err| eprintln!("Check error: {}", err)).unwrap();
            });
