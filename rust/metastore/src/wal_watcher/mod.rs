...
impl<'de> Visitor<'de> for EntryVisitor {
    ...
    fn visit_map<A>(self, mut map: A) -> Result<Self::Value, A::Error> where A: MapAccess<'de> {
        ...
        let (write, writeset): (&str, Vec<[&str; 2]>) = map.next_entry().unwrap().unwrap();

        assert_eq!(read, "read");
        assert_eq!(write, "write");

        let readset = readset.iter().map(|key| CustomKey(ObjectPath::new(key))).collect();
        let writeset = writeset.iter().map(|[key, value]| {
            CustomValue((
                CustomKey(ObjectPath::new(key)),
                ValueWithMVCC(MVCCMetadata::new_default(Timestamp::now()), value.to_string())))
        }).collect();
...
mod tests {
    ...
    fn wal_serialize() {
        ...
        let writes = vec![
            (ObjectPath::new("read1"), ValueWithMVCC(MVCCMetadata::default(), "fdsavcx".to_string())),
            (ObjectPath::new("read2"), ValueWithMVCC(MVCCMetadata::default(), "fdsavcx".to_string())),
            (ObjectPath::new("read3"), ValueWithMVCC(MVCCMetadata::default(), "fdsavcx".to_string()))];
