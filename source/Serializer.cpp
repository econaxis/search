...
uint32_t Serializer::read_num(std::istream& stream) {
    ...
    stream.read(buffer.get(), sizeof(uint32_t));
    auto value = *(reinterpret_cast<uint32_t*>(buffer.get()));
    return value;
}
...
WordIndexEntry Serializer::read_work_index_entry(std::istream& stream) {
    ...
    for(int i = 0; i<doc_pointer_len; i++) {
        docs.push_back({read_num(stream), static_cast<uint16_t>(read_num(stream))});
    }
